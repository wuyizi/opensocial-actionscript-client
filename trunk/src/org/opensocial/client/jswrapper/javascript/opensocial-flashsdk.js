...
opensocial.flash = (function() {
  ...
  var kernel = function() {
     ...
     * <p>
     * NOTE: Myspace is not support "params[fieldNames.PROFILE_DETAILS] = null", "params[fieldNames.SORT_ORDER] = null" and "params[fieldNames.SORT_ORDER]" ;
     * </p>
    ...
    me.translatePeopleRequestParams = function(options) {
      ...
      if (options['count']) {
        params[fieldNames.MAX] = options['count']
      }
      
      if (options['startIndex']) {
      	params[fieldNames.FIRST] = options['startIndex'];
      }
      
      if (options['sortBy']){
      	params[fieldNames.SORT_ORDER] = options['sortBy'];
      }
      
      if (options['filterBy']) {
      	params[fieldNames.FILTER] = options['filterBy'];
      }

      if (options['fields'] && options['fields'].length > 0) {
        params[fieldNames.PROFILE_DETAILS] = options['fields'];
      }
      return params;
    };
    ...
    me.api = /** @scope opensocial.flash */ {
       ...
       * need a special one.
       * <p>
       * NOTE: to be deprecated. If the container server is not Shindig,  there is not "gadgets.flash.getMajorVersion" function.
       * </p>
       *
      ...
      embedFlash : function(swfUrl, swfContainer,
        ...
        EXPORT("PLAYER_CURRENT_VER", opensocial.flash.getMajorVersion());
        EXPORT("PLAYER_EXPECED_VER", expectMajorVer);

        if (opensocial.flash.getMajorVersion() < expectMajorVer) {
      ...
      /**
       * Get the major version of the flash player.
       * @return {Number} The major version of Flash Player or 0 if Flash is not supported.
       * @member opensocial.flash
       */  
